    ...
    [(letA e1 e2)
     (let ([x (gensym)])
       (letE x (translate-back e1 env)
           (translate-back e2 (extend-envA env x))))])) ;envA to w tym przypadku envE, ale nie chcialo mi sie zmieniac nazwy.





;var to indeksy, aka liczba przeskoczen w gore


;(letA (numA 1) (opA (add) (varA 0) (letA (numA 2) (opA (mul) (varA 1) (varA 0)))))
;                     ------------------------------------------->
;(letE 'x (numE 1) (opE (add) (varE 'x) (letE 'y (numE 2) (opE (mul) (varE 'x) (varE 'y)))))

;ext-env (gensym)

;(letA (numA 1) (opA (add) (varA 0) (letA (opA (add) (varA 0) (numA 1)) (opA (mul) (varA 0) (numA 3)))))
;                     -------------------------------------------->
;(letE 'x (numE 1) (opE (add) (varE 'x) (letE 'x (opE (add) (varE 'x) (numE 1)) (opE (mul) (varE 'x) (numE 3))))
...
;`{let x 1
 ;    {+ x {let y {+ x 1}
  ;       {* y 3}}}}
;czyli gensyma mozemy uzywac za kazdym razem, gdy uzywamy let.
...
(define original (parse `{let x 1
   {+ x {let x {+ x 1}
      {* x 3}}}}))

(define translated (translate (parse `{let x 1
                {+ x {let x {+ x 1}
                       {* x 3}}}}) mt-env))


(define translated-back (translate-back translated mt-env))
  ...
  (test
   (eval (parse `{let x 1
                   {+ x {let x {+ x 1}
                          {* x 3}}}}) mt-env)
   (eval (translate-back
          (translate (parse `{let x 1
                               {+ x {let x {+ x 1}
                                      {* x 3}}}}) mt-env) mt-env) mt-env))

  (test
   (eval (parse `{let x 1 {+ x {let y 2 {* x y}}}}) mt-env)
   
   (eval (translate-back
          (translate (parse `{let x 1 {+ x {let y 2 {* x y}}}}) mt-env) mt-env) mt-env))

  (test
   (eval (parse `{+ 2 1}) mt-env)
   
   (eval (translate-back
          (translate (parse `{+ 2 1}) mt-env) mt-env) mt-env))
  )
