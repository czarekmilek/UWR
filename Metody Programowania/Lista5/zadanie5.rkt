...
(define (bst? t)
  (process-tree
        ;czy jesteśmy w range
        (lambda (acc l val r) (and (<= (fst acc) val) (>= (snd acc) val) l r))
        ; jesli doszlismy do leaf to zwracamy #t
        (lambda (leaf) #t)
        ;ograniczenie góra/dół
        (lambda (acc v) (pair (fst acc) v))
        ;Funkcja ta przyjmuje dwa argumenty: aktualny akumulator acc oraz wartość węzła v,
        ;a następnie tworzy nową parę, w której pierwszym elementem jest wartość minimalna
        (lambda (acc v) (pair v (snd acc)))
        ;Funkcja ta przyjmuje dwa argumenty: aktualny akumulator acc oraz wartość węzła v,
        ;a następnie tworzy nową parę, w której drugim elementem jest wartość maksymalna
        (pair -inf.0 +inf.0)
        t
       )
