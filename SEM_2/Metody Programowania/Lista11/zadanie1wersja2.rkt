...
(define-type Op
  (add) (sub) (mul) (div) (eql) (leq) (errOp)) ;NOWE
...
(define (parse-exp [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{try ANY ANY} s)            ;NOWE
     (tryE (parse-exp (second (s-exp->list s))) ;NOWE
           (parse-exp (third (s-exp->list s))))];NOWE
...
;testy
(define test1 (parse-exp `{try {if {{= 2 3 1} 3}} 2}))
(eval test1)

(define test2 (parse-exp `{try {cond {{= 2 2} 3 3}} 2}))
(eval test2)
...
(eval (parse-exp `{try {+ {error a "aaa"} 1} (errorerror a "AAAAAAA")}))
